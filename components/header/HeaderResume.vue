  ...
  lines.forEach((line) => {
    animate
      .from(line, 0.6, {
        ...
        width: 0,
        onUpdate() {
          line.parentElement.style = `border-right : 2px solid #333;width:${line.offsetWidth + 15}px`;
        },
      })
      .to(line, 0.6, {
        duration: 0.3,
      ...
      .to(line, 0, {
        ...
        onComplete() {
          line.parentElement.style = `width:0px`;
        },
      }, '+=0.2');
  });
});
  ...
  <header id="dsn_header" class="dsn-header-animation dsn-full-header v-dark-head">
    <div class="entry-header p-relative over-hidden">
      <div id="hero_image" class="p-absolute dsn-hero-parallax-img over-hidden" data-dsn-ajax="img"
        ...
        <img :src="xUrl(props.data.images[0].src)" class="cover-bg-img transform-3d" alt="" />
      </div>
              ...
              <div id="hero_title" class="mt-20">
                <h1 ref="headline" class="ah-headline title clip is-full-width">
                  ...
                  <span ref="ahWords" class="ah-words-wrapper">
                    <b>{{ props.data.name }}</b>
                    <b v-for="(item, index) in props.data.job" :key="index">{{ item }}</b>
                  </span>
                </h1>
              </div>
