...
const animateShow = gsap.timeline();
const animateHide = gsap.timeline();

onMounted(() => {
	const wrap = document.body.querySelector("#main_root");
	if (!wrap) return;
	const boxNumber = scroll.value.querySelector(".box-number >span");
	animate.to(svg.value, 0.3, { rotation: wrap.scrollHeight / 2 });
	ScrollTrigger.create({
		...
		scrub: true,
		onUpdate($s) { boxNumber.innerText = `${($s.progress.toFixed(3) * 100).toFixed(0)}%`; },
		onEnter: Show,
		onEnterBack: Show,
		onLeave: Hide,
		onLeaveBack: Hide,
	});
	function Show() { animateShow.to(scroll.value, 1, { right: 10, autoAlpha: 1 }); }
	function Hide() { animateHide.to(scroll.value, 1, { right: -100, autoAlpha: 0 }); }

	const scrollCurrent = scroll.value;
	const handleClick = () => {
		if (pStore.activeScrollbar) {
...
onUnmounted(() => {
	...
	if (animateShow.scrollTrigger) animateShow.scrollTrigger.kill();
	animateShow.kill();

	if (animateHide.scrollTrigger) animateHide.scrollTrigger.kill();
	animateHide.kill();
});
</script>

<template>
	<div class="scroll-to-top" ref="scroll">
		<svg version="1.1" xmlns="http://www.w3.org/2000/svg" ref="svg" xmlnsXlink="http://www.w3.org/1999/xlink"
			x="0px" y="0px" viewBox="0 0 174 173" xmlSpace="preserve">
