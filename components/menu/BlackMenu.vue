<template>
	<Navbar ref="nav" :hamburger="props.hamburger" :removeOpenMenu="removeOpenMenu">
...
const store = reactive({ navBar: Element });
const removeOpenMenu = () => {
	const ul = nav.value.nav.querySelectorAll("ul");
	if (ul) {
		nav.value.nav
			.querySelectorAll("ul")
...
//--------------------------------------------------------------------------------------------
const cutterElement = () => {
	nav.value.nav.querySelectorAll("ul").forEach((item, index) => {
		item.style.setProperty("--dsn-li-name", `dsn${index}`);
		Object.keys(item.children).forEach(($key) => { item.children[$key].style.setProperty("--dsn-li-index", $key); });
	});
};
//--------------------------------------------------------------------------------------------
onMounted(() => {
	store.navBar = nav.value.nav;
	cutterElement();
});
</script>
