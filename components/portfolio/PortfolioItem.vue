...
const setImageParallax = '{"trigger": { "start": "-80% top", "end": "100%" },"from": { "y": "-20%", "height": "120%" },"to": { "y": "0%", "height": "120%" }}';
...
<template>
    <props.tag v-for="(item, index) in props.data" :key="index" :class="props.className,
        !props.mode ? 'v-dark' : '',
        ...
        <div class="box-content d-flex">
            <NuxtLink class="box-image-link bg-shadow effect-ajax" v-if="item.link" :to="item.link + item.slug" data-dsn-ajax="work"
                title="HEADPHONES">
                <div class="box-image-bg before-z-index dsn-swiper-parallax-transform" data-overlay="4"
                    ...
                    <img class="cover-bg-img" :src="xUrl(item.images[0].src)" alt="" />
                </div>
            </NuxtLink>
            <div class="post-content p-relative z-index-1 d-flex flex-column dsn-swiper-parallax-transform "
                :class="props.postContentClass" v-bind="props.postContentParallax">
                <div class="post-title-info">
                    <div class="post-meta max-w750">
                        <MetaPost className="mb-10" v-if="item.categories" :category="item.categories" catLink="/work/category/" separate=" &" />
                    </div>
                    <h2 class="post-title" v-if="item.title" :class="props.postTitleClass">
                        <NuxtLink class="effect-ajax" :to="item.link + item.slug">{{ item.title }}</NuxtLink>
                    </h2>
                </div>

                <div class="post-description-info" v-if="
                    props.itemDescription || props.showDescription || props.linkCircle
                ">
